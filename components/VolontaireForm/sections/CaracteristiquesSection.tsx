...
const SOUS_ETHNIE_LABELS: Record<string, string> = {
    ...
    'MEDITERRANEEN/MAGHREBIN': 'Méditerranéen / Maghrébin',
...
const YEUX_OPTIONS = ['Bleus', 'Verts', 'Marrons', 'Noisette', 'Gris', 'Noirs'];

const PILOSITE_OPTIONS = [
    { label: 'Faible', value: 'Faible_pilosite' },
    { label: 'Moyenne', value: 'Moyenne_pilosite' },
    { label: 'Forte', value: 'Forte_pilosite' },
];

const parseCSV = (val: unknown): string[] => {
    if (!val) return [];
    if (Array.isArray(val)) return val;
    return String(val).split(',').filter((entry) => entry.trim() !== '');
};
...
const CaracteristiquesSection: React.FC<SectionProps> = ({
    ...
    const handleSousEthnieToggle = (sousEthnie: string) => {
        const newSousEthnies = sousEthniesSelectionnees.includes(sousEthnie)
            ? sousEthniesSelectionnees.filter(se => se !== sousEthnie)
            : [...sousEthniesSelectionnees, sousEthnie];

        handleChange('sousEthnie', newSousEthnies.join(','));
    };

    const sousEthniesDisponibles = ethniesSelectionnees.flatMap(
        ethnie => SOUS_ETHNIES_PAR_ETHNIE[ethnie] || []
    );
